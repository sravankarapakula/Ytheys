    ...
    const fetchAgencyData = async () => { 
      ...
      try {
        const fetched = await Promise.all(
          agencyData.map(async (agency, index) => {
            ...
            return {
              id: encodeURIComponent(agency.repo),
          ...
          <div className="flex flex-col gap-5 w-full">
            ...
            <div className="relative w-full max-w-[22rem]">
              ...
              <input
                ...
                placeholder="Find agencies by service or domain"
            ...
            <table className="min-w-full table-auto border-collapse bg-black/40 backdrop-blur-sm border border-neutral-800/50 overflow-hidden">
              ...
              <tbody>
                {filtered.map((agency, idx) => ( 
                  <tr 
                    key={idx} 
                    onClick={() => router.push(`/startup/${agency.id}`)}
                    className="border-b border-neutral-800/30 group hover:bg-neutral-900/20 transition cursor-pointer"
                  >
                    {columns.map(({ key }) => (
          ...
          <div className="md:hidden space-y-4">
            {filtered.map((agency, idx) => ( 
              <div 
                key={idx} 
                onClick={() => router.push(`/startup/${agency.id}`)}
                className="border border-neutral-800/50 p-4 bg-black/40 backdrop-blur-sm space-y-5 hover:border-neutral-700 transition cursor-pointer"
              >
